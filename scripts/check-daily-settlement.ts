...
const noActivity = settlementPeriod(
  ...
  [],
  { TWD: 2 },
  { 'merchant:M:PV00001:TWD': 250000000 },
  '2026-10-01'
)
assert.equal(noActivity.lines.length, 0)
assert.equal(noActivity.carry[0].closing, 250000000)
...
assert.equal(run(negative)[0].settled, -448000000)
assert.equal(run(negative)[1].base, 20000)
const scope = rows[0].scope
const opening = run(example.bets, fx, costs, { [scope]: 25000 })
assert.equal(opening[0].used, 0) // Carry is consumed at period level, after FX netting.
assert.equal(opening[1].used, 0)
assert.equal(opening[1].base, 20000)
...
assert.equal(providerCalc('zero')[0].settled, -320000000)
assert.equal(providerCalc('carry')[0].added, 0)
assert.equal(providerCalc('carry')[1].base, 20000)
const tiny = [{ ...example.bets[0], bet: 100, valid: 100, payout: 99 }]
assert.equal(run(tiny)[0].settled, 22400) // 0.01 × 7% × 32 = 0.022400; retain six decimals.
console.log(
  'Daily detail passed: settlement-date FX, six-digit precision, missing/conflicting FX, scoped carry and duplicates'
)

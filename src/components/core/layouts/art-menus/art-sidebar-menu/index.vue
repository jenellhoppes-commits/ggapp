...
<template>
  <div
    ...
    :class="{ 'no-border': menuList.length === 0, 'section-sidebar': useSections }"
  >
    <!-- 双列菜单（左侧） -->
    <div
      v-if="isDualMenu"
        ...
        <ul>
          <li v-for="menu in firstLevelMenus" :key="menu.path" @click="handleMenuJump(menu, true)">
            ...
            >
              <div
                :class="{
                  'is-active': !menu.children?.length
    ...
    >
      ...
      <ElScrollbar :style="scrollbarStyle" :inert="useSections && isMobileScreen && !menuOpen">
        <GgapSectionMenu
          v-if="useSections"
...
<script setup lang="ts">
  ...
  import { buildDisplayMenu } from '@/utils/navigation/display-menu'
  import { isIframe } from '@/utils/navigation'
  import { handleMenuJump } from '@/utils/navigation'
  import SidebarSubmenu from './widget/SidebarSubmenu.vue'
  import GgapSectionMenu from './widget/GgapSectionMenu.vue'
  ...
  const rawMenus = computed(() => useMenuStore().menuList)
  const useSections = computed(
    () => !isDualMenu.value && rawMenus.value.some((r) => r.name === 'OverviewSection')
  )
  ...
  const firstLevelMenuPath = computed(
    () =>
      firstLevelMenus.value.find(
        (menu) =>
          menu.path === routerPath.value ||
          route.matched.some((matched) => matched.path === menu.path)
      )?.path
  )
  ...
  const firstLevelMenus = computed(() => {
    return buildDisplayMenu(useMenuStore().menuList)
  })

  const menuList = computed(() => {
    const menuStore = useMenuStore()
    const allMenus = buildDisplayMenu(menuStore.menuList)
    ...
    // 返回当前顶级路径对应的子菜单
    const currentMenu = allMenus.find((menu) => menu.path === firstLevelMenuPath.value)
  ...
  const handleMenuClose = (): void => {
    if (isMobileScreen.value) {
      ...
      delayHideMobileModal()
      document.getElementById('main-navigation-toggle')?.focus()
    }
  }
...
<style lang="scss" scoped>
  ...
  .section-sidebar .header p {
    font-size: 16px;
    right: 8px;
    white-space: nowrap;
  }
  .section-sidebar .menu-left-close .header p {
    display: none;
  }
</style>

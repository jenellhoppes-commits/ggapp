...
import { legacySummaryTarget } from '@/domain/report-navigation'
...
async function handleRouteGuard(
  ...
  if (to.matched.length > 0) {
    ...
    if (
      allowedRoles?.length &&
      !allowedRoles.some((role) => userStore.info.roles?.includes(role))
    ) {
      next({ name: 'Exception403', replace: true })
      return
    }
    const legacySummary = legacySummaryTarget(to.path, to.query)
    if (legacySummary) {
      next({
        ...legacySummary,
        hash: to.hash,
